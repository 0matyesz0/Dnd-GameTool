...
class ShowMapActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // seperating a huge image file into multiple tiles
        val tileStreamProvider = object : TileStreamProvider {
            override fun getTileStream(row: Int, col: Int, zoomLvl: Int): InputStream? {
                return assets?.open("map_tiles/tile-${col}_${row}.jpg")
            }
        }

        // configurating the mapView
        val config = MapViewConfiguration(levelCount = 1, fullWidth = 10200, fullHeight = 6600,
            tileSize = 256, tileStreamProvider = tileStreamProvider)
            .setMaxScale(2f)

        mapView.configure(config)
        fullscreen_content.addView(mapView)
    }
